...
class StoreRepository @Inject constructor(
    ...
    private var cachedStores: List<StoreResponse>? = null
     ...
     * Get all stores
     */
    suspend fun getAllStores(forceRefresh: Boolean = false): NetworkResult<List<StoreResponse>> {
     ...
     * Get store by ID
     */
    suspend fun getStoreById(storeId: String): NetworkResult<StoreResponse> {
        // Check cache first
        cachedStores?.find { it.storeId == storeId }?.let {
            return NetworkResult.Success(it)
        }
    ...
    suspend fun getNearbyStores(
        ...
        longitude: Double,
        radius: Double = 10.0
    ): NetworkResult<List<StoreResponse>> {
        return safeApiCallWithRetry(errorHandler) {
     ...
     */
    suspend fun searchStores(query: String): NetworkResult<List<StoreResponse>> {
        return safeApiCallWithRetry(errorHandler) {
     ...
     */
    fun getCachedStores(): List<StoreResponse>? = cachedStores
}

...
class MenuBrowseViewModel: ObservableObject {
    ...
    // Define category order to match backend
    ...
    func loadAllMenuItems() async {
        ...
        do {
            ...
            // Extract unique categories and sort by predefined order
            let uniqueCategories = Set(allMenuItems.map { $0.category })
            categories = categoryOrder.filter { uniqueCategories.contains($0) }
